...
class EnvironmentParameterSettings:
    ...
    def structure(d: Mapping, t: type) -> Any:
        ...
        if "trainer_type" not in d_copy:
            raise TrainerConfigError("Trainer type not specified in the YAML file.")
        
        trainer_type = d_copy["trainer_type"]
        print(f"DEBUG: Trainer type in YAML: {trainer_type}", flush=True)  # Add this

        if trainer_type not in all_trainer_types:
            print(f"DEBUG: Trainer type '{trainer_type}' not in all_trainer_types:", all_trainer_types.keys(), flush=True)
            raise TrainerConfigError(f"Invalid trainer type {trainer_type} found.")

        d_copy["hyperparameters"] = check_hyperparam_schedules(
            d_copy.get("hyperparameters", {}), trainer_type
        )

        try:
            d_copy["hyperparameters"] = strict_to_cls(
                d_copy["hyperparameters"], all_trainer_settings[trainer_type]
            )
        except KeyError:
            raise TrainerConfigError(
                f"Settings for trainer type {trainer_type} not found."
            )
        return t(**d_copy)
...
class TrainerSettings(ExportableSettings):
    ...
    def structure(d: Mapping, t: type) -> Any:
        ...
        print("DEBUG: TrainerSettings.structure() called")
        """
        ...
        for key, val in d_copy.items():
            ...
            #         d_copy["checkpoint_interval"] = int(d_copy["max_steps"] / d_copy["keep_checkpoints"])
                ...
                print(f"DEBUG: Trainer type in YAML: {val}", flush=True)
                if val not in all_trainer_types.keys():
                    print("DEBUG: Available trainer types:", all_trainer_types.keys(), flush=True)
